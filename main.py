...
if not os.path.isfile('my_mod_el.pt'):
    ...
    for episode in tqdm(range(episodes)):
        step = 0
        state, info = env.reset()
        if not np.isin(state, agent.all_state).all(): 
        ...
        while not done and step < max_steps:
            ...
            if not np.isin(next_state, agent.all_state).all(): 
...
def find_array_index(arr_list, arr):
    for i, a in enumerate(arr_list):
        ...
        #    return i
        return i if np.array_equal(a, arr) else -1  # Si le tableau n'a pas été trouvé dans la liste, retourne -1
